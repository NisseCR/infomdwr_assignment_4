...
For pre-processing, the reviews are converted to lowercase to ensure consistency in word comparison. Next, whitespaces, punctuation, and numbers are removed. The reviews are then lemmatized to reduce words to their root forms; for example, “changed” becomes “change.” After lemmatization, stemming is applied to strip word suffixes, so a word like “generous” is reduced to “gener.” Finally, stopwords are removed from the text. These include common English words such as “are” and “I,” along with custom stopwords identified during EDA, like “movie” and “film.” 
...
```{r}
gmm_10_plt <-plot_ly(df, x = ~x, y = ~y, type = "scatter", color = ~movie_review$sentiment) %>%
  layout(annotations = list(text = "10 GMM",showarrow = F))
gmm_10_plt
```
...
cat("K-Means (5 clusters):", kmeans_5_sil_score, "\n")
cat("K-Means (10 clusters):", kmeans_10_sil_score, "\n")
cat("GMM (5 clusters):", gmm_5_sil_score, "\n")
cat("GMM (10 clusters):", gmm_10_sil_score, "\n")
```

# Evaluation of clustering results Using the Silhouette Index
...
K-Means with 5 clusters: 0.0599
K-Means with 10 clusters: 0.0305
GMM with 5 clusters: 0.0846
GMM with 10 clusters: 0.0737
These scores are relatively low, suggesting that the clusters are not well-separated, and may reflect the inherent complexity and overlapping themes within movie reviews. Interestingly, the GMM approach yielded slightly higher scores than K-Means for both cluster settings, implying that GMM might be slightly better suited for this dataset. However, both models struggled to produce highly distinct clusters, indicating that additional tuning or alternative methods may be required to improve clarity in grouping similar reviews.
